...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
	...
	// Create camera
	camera = new Camera();
	camera->lookAt(Vector3(-0.0f, 0.0f, 22.f), Vector3(0.f, 0.0f, 0.f), Vector3(0.f, 1.f, 0.f));
	...
	//create volume and volume texture
	...
	VolumeMaterial* vol_material = new VolumeMaterial(tex);

	//create node
	SceneNode* node = new SceneNode("Volume");
	node->mesh = cube;
	node->material = vol_material;
	node->model.setScale(5.0, 5.0, 9.0);
...
void Application::update(double seconds_elapsed)
{
	...
	for (int i = 0; i < node_list.size(); i++) {
		node_list[i]->update(seconds_elapsed);
	}
}
...
void Application::onKeyDown( SDL_KeyboardEvent event )
{
	switch(event.keysym.sym)
	{
		...
		case SDLK_1: std::cout << HDRE::sHDRELoaded.size() << std::endl; break;
	}
}
